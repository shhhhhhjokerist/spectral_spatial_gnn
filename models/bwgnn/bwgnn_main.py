...
def train(model, g, args):
    ...
    # graph_t = amazon_graph(True, sample_num_nodes=1000, num_edges=1000*30, anomaly_ratio=0.2, hetero_ratio=0.5, homo_edge_anomaly_ratio=0.5, homo=True)
    # model.eval()
    # features_t = graph_t.ndata['feature']
    # labels_t = graph_t.ndata['label']
    #
    # logits = model(features_t)
    #
    #
    # probs = logits.softmax(1)
    # f1, thres = get_best_f1(labels_t, probs)
    # preds = numpy.zeros_like(labels_t)
    # preds[probs[:, 1] > thres] = 1
    # trec = recall_score(labels_t, preds)
    # tpre = precision_score(labels_t, preds)
    # tmf1 = f1_score(labels_t, preds, average='macro')
    # tauc = roc_auc_score(labels_t, probs[:, 1].detach().numpy())
    # print('tttttest mf1: {:.4f}, auc {:.4f}'.format(tmf1, tauc))
