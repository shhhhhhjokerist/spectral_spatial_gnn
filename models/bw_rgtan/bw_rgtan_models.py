...
class RGTAN(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Linear(hidden_dim*4*2, hidden_dim*4)
        self.dropout = nn.Dropout(0.5)
        self.fc2 = nn.Linear(hidden_dim*4, 2)  # 二分类

    def forward(self, blocks, features, labels, h_bw, n2v_feat=None, neighstat_feat=None):
        """
        ...
        h = torch.cat([h_bw, h], dim=1)  # [N, d1 + d2]
        h = F.relu(self.fc1(h))
        h = self.dropout(h)
...
class BW_RGTAN_AGG(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, dropout=0.5):
        super(BW_RGTAN_AGG, self).__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.dropout = nn.Dropout(dropout)
        self.fc2 = nn.Linear(hidden_dim, 2)  # 二分类

    def forward(self, h_bw, h_rgtan):
        h = torch.cat([h_bw, h_rgtan], dim=1)  # [N, d1 + d2]
        h = F.relu(self.fc1(h))
        h = self.dropout(h)
        out = self.fc2(h)  # logits
        return out
