...
from models.chebnet import models
...
def process_data(device, dataset, gso_type):
    if dataset == 'amazon':
        # feature, adj, label, idx_train, idx_val, idx_test, n_feat, n_class = dataloader.load_dgl_data(dataset)

        graphs, label_dict = dgl.load_graphs('../data/graph_data/amazon/org_graph.dgl')
        graph = graphs[0]
        ...
        idx_train = train_mask.nonzero().squeeze()
        idx_val = val_mask.nonzero().squeeze()
        idx_test = test_mask.nonzero().squeeze()
        ...
        data = np.ones(len(u))  # 假设所有边的权重为1
        adj = sp.csc_matrix((data, (u, v)), shape=(label.shape[0], label.shape[0]))
